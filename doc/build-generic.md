GENERIC BUILD NOTES
====================
Some notes on how to build StakeCubeCoin Core based on the [depends](../depends/README.md) build system.

Note on old build instructions
------------------------------
In the past, the build documentation contained instructions on how to build StakeCubeCoin with system-wide installed dependencies
...
------------------------------------
Building the dependencies and StakeCubeCoin Core requires some essential build tools to be installed before. Please see
[build-unix](build-unix.md), [build-osx](build-osx.md) and [build-windows](build-windows.md) for details.

Building dependencies
---------------------
StakeCubeCoin inherited the `depends` folder from Bitcoin, which contains all dependencies required to build StakeCubeCoin. These
dependencies must be built before StakeCubeCoin can actually be built. To do so, perform the following:
...
This will download and build all dependencies required to build StakeCubeCoin Core. Caching of build results will ensure that only
...
not done, build failures might occur when building StakeCubeCoin.
...
ccache
------
`./configure` of StakeCubeCoin Core will autodetect the presence of ccache and enable use of it. To disable ccache, use
