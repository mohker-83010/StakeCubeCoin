...
The data directory is the default location where the SCC Core files are stored.
...
3. All content of the data directory, except for `scc.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`llmq/`          |                       |quorum signatures database
`./`               | `banlist.dat`         | Stores the IPs/subnets of banned nodes
`./`               | `scc.conf`        | Contains [configuration settings](scc-conf.md) for `sccd` or `scc-qt`; can be specified by `-conf` option
`./`               | `sccd.pid`        | Stores the process ID (PID) of `sccd` or `scc-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `sccd` or `scc-qt`; can be specified by `-debuglogfile` option
`./`               | `governance.dat`      | stores data for governance objects
`./`               | `mncache.dat`         | stores data for masternode list
`./`               | `netfulfilled.dat`    | stores data about recently made network requests
`./`               | `fee_estimates.dat`   | Stores statistics used to estimate minimum transaction fees and priorities required for confirmation
...
These subdirectories and files are no longer used by the SCC Core:
