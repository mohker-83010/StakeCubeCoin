 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called StakeCubeCoin (https://www.stakecube.net/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    if (gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||  gArgs.IsArgSet("-help") || gArgs.IsArgSet("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  sccd [options]                     " + strprintf(_("Start %s Daemon"), _(PACKAGE_NAME)) + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                fprintf(stderr, "Error: Command line contains unexpected token '%s', see sccd -h for a list of options.\n", argv[i]);
                return false;
            }
        }
