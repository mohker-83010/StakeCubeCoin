                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in StakeCubeCoin
...
SCC :   We keep the logic for now and just add +1 to match current height, so we only need to make changes here.

        First block contains 7.5 million SCC to swap. Remaining coins after get burned swap period.        
        TODO: add burn TX and amount here for reference
*/
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight <= 200)       { ret = blockValue * 0; // MN payments start at block 200
    } else if (nHeight > 200) { ret = blockValue * 0.70; }
...
{
    LogPrintf("%s: conflicting block=%s  height=%d  log2_work=%.8f  time=%d\n", __func__,
      pindexNew->GetBlockHash().ToString(), pindexNew->nHeight,
      log(pindexNew->nChainWork.getdouble())/log(2.0), pindexNew->GetBlockTime());
    CBlockIndex *tip = chainActive.Tip();
    assert (tip);
    LogPrintf("%s:  current best=%s  height=%d  log2_work=%.8f  time=%d\n", __func__,
      tip->GetBlockHash().ToString(), chainActive.Height(), log(tip->nChainWork.getdouble())/log(2.0),
      tip->GetBlockTime());
    CheckForkWarningConditions();
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // StakeCubeCoin : Check superblock start
    ...
    // StakeCubeCoin : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(SCC): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(SCC): spork is off, skipping transaction locking checks\n");
    }
    ...
    // StakeCubeCoin : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            if (fFailedChain || fMissingData || fConflictingChain) {
                // Candidate chain is not usable (either invalid, conflicting or missing data)
